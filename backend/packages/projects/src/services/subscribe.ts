...
const call = (self: Self): SubscribeService['call'] => async (input) => {
    ...
    await self.redisClient.SADD(`subscriptions:resources:projects:${userId}`, connectionId)
    await self.redisClient.SADD(`subscriptions:connections:projects:${connectionId}`, userId)
