...
const call = (self: Self): SubscribeService['call'] => async (input) => {
    ...
    await self.redisClient.SADD(`subscriptions:resources:visualisations:${projectId}`, connectionId)
    await self.redisClient.SADD(`subscriptions:connections:visualisations:${connectionId}`, projectId)
